...
@WebServlet("/addElectronicsServlet")
public class AddElectronicsServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int count = AdminUtils.addElectronics(electronics);

        PrintWriter out = resp.getWriter();
        if (count > 0) {
            out.println("<html><script type='text/javascript' src='js/add_electronics_success.js'></script></html>");
        } else {
            out.println("<html><script type='text/javascript' src='js/add_electronics_failure.js'></script></html>");
        }
        out.close();
    }
}
